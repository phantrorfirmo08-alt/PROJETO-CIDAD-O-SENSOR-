...
class CidadaoSensorApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      // Tema Escuro (Descansar a vista / Noturno)
      darkTheme: ThemeData(
      ...
      themeMode: ThemeMode.system, // Alterna automaticamente conforme o celular do usuário
...
// 2. Tela Principal do Painel de Monitoramento
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              if (pinController.text == '123456') { // PIN padrão configurável
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('PIN Incorreto! Acesso negado.')),
                );
              }
            },
  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: [
        ...
        const SizedBox(height: 20),
        const Text(
          'Mapa de Risco e Sensores Comunitários',
          ...
          child: const Center(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 8),
                Text('Visualização do Mapa Georreferenciado'),
              ],
            ),
          ),
        ),
        ...
        ElevatedButton.icon(
          ...
          onPressed: () {
            // Ação de reporte comunitário por áudio/relato
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('Canal de Relato Rápido acionado.')),
            );
          },
...
class CircuitoSocialTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: const [
        Text(
          'Circuito Social - Eventos e Avisos',
          style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold),
        ),
        SizedBox(height: 10),
        Card(
          child: ListTile(
            ...
            title: Text('Reunião Comunitária - Bacia do Sarapuí'),
            subtitle: Text('Atualizações sobre o programa Cidade Resiliente.'),
          ),
        ),
        Card(
          child: ListTile(
            leading: Icon(Icons.video_library, color: Colors.blue),
            title: Text('Aviso Geral: Cadastramento Municipal'),
            subtitle: Text('Informações integradas sobre habitação e suporte.'),
          ),
        ),
      ],
    );
      ...
      body: ListView(
        ...
        children: [
          ...
          ElevatedButton.icon(
            ...
            label: const Text('Gerenciar Alertas de Inundação'),
          ),
          const SizedBox(height: 10),
          ElevatedButton.icon(
            onPressed: () {},
            icon: const Icon(Icons.video_call),
            label: const Text('Atualizar Vídeos do Circuito Social'),
          ),
          const SizedBox(height: 10),
          ElevatedButton.icon(
            ...
            label: const Text('Forçar Execução do monitor.py'),
          ),
        ],
      ),
    );
...
class AppBarTermos {
  static AppBar call({required Text title}) {
